...
lowerbones = np.array(lowerbones) - 1
    ...
    while (len(occb) < (N*2)):
        if len(occb) < (H*2):
            ...
            for i in range(0, G):
                ran = math.ceil(random.uniform(0, 3))
                if len(occb) < (N*2):
                    occb.extend(otherbody[rg][ran])

    # print('{} {}'.format('Number of occluded bones: ', len(occb)/2))
    print('{} {}'.format('Bones Occluded [Joints connected]: ', occb))
    # print(N, S)
    return occb
...
def addnoise(f, dt):
    ...
    # LOAD DATA AND LABELS
    data = directory + f + '.npy'
    actions = np.load(data) #np.asarray(np.float32(dt))
    print(actions.shape)
    for a in actions:
        action = []
        for cr in a:
